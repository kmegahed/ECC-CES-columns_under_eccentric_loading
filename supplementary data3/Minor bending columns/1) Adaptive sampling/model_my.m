function [force, check] = model(X)
    ...
    for i = 1:numel(B1)
        ...
        d_steel = d_steel1(i);    ro_rebar = ro_rebar1(i);    tf_tw2 = tf_tw1(i);    cH = cH1(i);    cB = cB1(i);ee=eeee1(i);eff=exp(ee)-0.9;ec=(eff)*B;
        new_jjj = sprintf('%d_%d_%d_%dy', round(B), round(H), round(lam), round(eff*100));%'%d_%0.2f_%0.1f_%d'
        % Modify and write the first script
        inputFilePath = 'C:\Users\osama\OneDrive\Desktop\abaqus2\script_my.py';
